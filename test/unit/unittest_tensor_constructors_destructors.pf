...
contains
  ...
  subroutine test_torch_tensor_zeros(this)
    ...
    ! Check the stride and shape
    @assertEqual(expected_stride, tensor1 % get_stride())
    @assertEqual(tensor_shape, tensor1 % get_shape())
  ...
  subroutine test_torch_from_array_1d(this)
    ...
    ! Check the stride and shape
    @assertEqual([1], tensor1 % get_stride())
    @assertEqual([6], tensor1 % get_shape())
  ...
  subroutine test_torch_from_array_2d(this)
    ...
    integer(c_int64_t) :: expected_stride(ndims)
    ...
    ! Verify the stride and shape
    @assertEqual(expected_stride, tensor1 % get_stride())
    @assertEqual(shape(in_data), tensor1 % get_shape())
  ...
  subroutine test_torch_from_array_3d(this)
    ...
    if (this%param%reordered) then
      tensor_layout(:) = [3, 2, 1]
      expected_stride(:) = [6, 3, 1]
    else
      tensor_layout(:) = [1, 2, 3]
      expected_stride(:) = [1, 1, 2]
    end if
    call torch_tensor_from_array(tensor1, in_data, tensor_layout, device_type, device_index, &
                                 this%param%requires_grad)
