...
FTorch provides a [[ftorch_tensor(module):torch_tensor(type)]] derived type,
...
Under the hood, the [[ftorch_tensor(module):torch_tensor(type)]] type holds a
...
We provide several subroutines for constructing
[[ftorch_tensor(module):torch_tensor(type)]] objects. These include:

* [[ftorch_tensor(module):torch_tensor_empty(subroutine)]] which allocates
  memory for the [[ftorch_tensor(module):torch_tensor(type)]] but does not set
  any values.
* [[ftorch_tensor(module):torch_tensor_zeros(subroutine)]] which creates a
  [[ftorch_tensor(module):torch_tensor(type)]] whose values are uniformly zero.
* [[ftorch_tensor(module):torch_tensor_ones(subroutine)]] which creates a
  [[ftorch_tensor(module):torch_tensor(type)]] whose values are
  uniformly one.
* [[ftorch_tensor(module):torch_tensor_from_array(interface)]] which allows the
  user to create a [[ftorch_tensor(module):torch_tensor(type)]] with the same
  rank, shape, and data type as a given Fortran array. Note that the data is
  *not* copied - the tensor data points to the Fortran array, meaning the array
  must have been declared with the `target` property. The array will continue to
...
We provide several subroutines for interrogating
[[ftorch_tensor(module):torch_tensor(type)]] objects. These include:

* [[ftorch_tensor(module):torch_tensor_get_rank(function)]] which determines the
...
* [[ftorch_tensor(module):torch_tensor_get_dtype(function)]] which determines
  ...
  `torch_kFloat32`, etc.
* [[ftorch_tensor(module):torch_tensor_get_device_type(function)]], which
  determines the device type that the tensor resides on in terms of the enums
  `torch_kCPU`, `torch_kCUDA`, `torch_kXPU`, etc.
* [[ftorch_tensor(module):torch_tensor_get_device_index(function)]], which
...
with a [[ftorch_tensor(module):torch_tensor(type)]] object:

* [[ftorch_tensor(module):torch_tensor_zero(subroutine)]] (aliased as class
  method `torch_tensor%zero`), which sets all the data entries associated with a
  tensor to zero.
...
[[ftorch_tensor(module):torch_tensor(type)]]s `a` and `b`,
```fortran
b = a
```
...
using [[ftorch_tensor(module):torch_tensor_empty(subroutine)]] under the hood
(inheriting all the properties of the tensors being added[^1]).
Following this, the intermediate tensor is assigned to `c`.
Finally, the finalizer for [[ftorch_tensor(module):torch_tensor(type)]] is
...
a [[ftorch_tensor(module):torch_tensor(type)]], an intermediate
[[ftorch_tensor(module):torch_tensor(type)]] will be created, assigned, and
