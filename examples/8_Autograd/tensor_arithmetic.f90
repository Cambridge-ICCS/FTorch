program autograd_tensor_arithmetic
  ...
  if (.not. assert_allclose(out_data1, expected, test_name="autograd_tensor_arithmetic_Q")) then
  ...
  if (.not. assert_allclose(out_data2, expected, test_name="autograd_tensor_arithmetic_dQdb")) then
    write(*,*) "Error :: value of dQ/db does not match expected value"
    stop 999
  end if
  write(*,*) "dQ/db = - 2*b = ", out_data3
  expected(:) = [-12.0_wp, -8.0_wp]
  if (.not. assert_allclose(out_data3, expected, test_name="autograd_tensor_arithmetic_dQdb")) then
    write(*,*) "Error :: value of dQ/db does not match expected value"
    stop 999
  end if

  write (*,*) "autograd_tensor arithmetic Autograd example ran successfully"
